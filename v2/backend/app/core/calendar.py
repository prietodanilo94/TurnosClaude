...
def build_solver_input(request: OptimizeRequest) -> SolverInput:
    ...
    for s in request.shift_catalog:
        shifts.append(ShiftInfo(
            id=s.id,
            nombre_turno=s.nombre_turno,
            rotation_group=s.rotation_group,
            horario_por_dia={day: {"inicio": hd.inicio, "fin": hd.fin} for day, hd in s.horario_por_dia.items()},
            dias_aplicables=s.dias_aplicables,
    ...
    return SolverInput(
        rotation_group=request.rotation_group,
        workers=workers,
        days=days,
        shifts=shifts,
        weeks=weeks,
