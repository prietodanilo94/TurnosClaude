"""
Helper para buscar áreas en el catálogo local (backend).
Espejo del seed-area-catalog.ts — mantener sincronizado.
"""
from typing import Optional
from app.models.schemas import AreaCatalog, Clasificacion, TipoFranja

# Catálogo local: codigo_area → datos del área
...
def lookup_area(codigo_area: str) -> Optional[AreaCatalog]:
    """Busca un área por su código numérico. Retorna None si no existe."""
    entry = _CATALOG.get(codigo_area.strip())
    if not entry:
        return None
    return AreaCatalog(id=codigo_area.strip(), **entry)
...
def get_rotation_group(
    ...
    # postventa
    if clasificacion == "standalone":
        return "P_SA"
    if clasificacion in ("mall_sin_dom", "mall_7d"):
        if comuna == "Oeste":                             return "P_MO"
        if comuna == "Tobalaba":                          return "P_MT"
        if comuna in ("Quilín", "Movicenter"):            return "P_MQ"
