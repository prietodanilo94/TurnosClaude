...
> Update 2026-04-24: el playground de `v3` ya no usa solo una rotacion fija; `v3/frontend/src/lib/optimizer-lab/engine.ts` ahora genera propuestas semanales reales de 42h, distribuye domingos visibles segun dotacion y valida cobertura minima completa en el rango extendido. Tambien quedaron tests del engine (`engine.test.ts`) y build real pasando.
> Update 2026-04-24: el playground de `v3` ahora deja elegir en la misma interfaz entre `Heuristico` y `OR-Tools CP-SAT`. El modo exacto se cableo para llamar por HTTP al optimizer real de `v2` cuando este disponible; si no lo esta, la UI devuelve diagnostico claro.
> Update 2026-04-24: se agrego el stack minimo de despliegue de `v3` (`v3/frontend/Dockerfile`, `v3/docker-compose.yml`, `v3/.env.example`) para publicarlo como frontend independiente en `:3013`, reutilizando el optimizer exacto de `v2` por HTTP.
> Update 2026-04-24: el enlace remoto por defecto entre `v3` y el optimizer exacto de `v2` se fijo en `https://turnos2.dpmake.cl/api`, porque el bind local `127.0.0.1:8022` del host no es visible desde el contenedor de `v3`.
> Update 2026-04-24: `v3` quedo publicado en `https://turnos3.dpmake.cl` y su playground `/admin/optimizer-lab` ya consume el optimizer exacto de `v2` via el endpoint publico `https://turnos2.dpmake.cl/api`.
> Update 2026-04-24: validacion publica del playground de `v3` completada: `POST /api/optimizer-lab` sobre `turnos3` respondio en ambos modos (`heuristic` y `cp_sat`), y el modo exacto devolvio diagnostico `ILP status: OPTIMAL` reutilizando `v2`.
