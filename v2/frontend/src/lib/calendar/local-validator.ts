...
function checkHorasSemanales(ctx: ValidateContext): Violation[] {
  ...
  for (const [rut, weeks] of Object.entries(horasPorRutSemana)) {
    for (const [wk, hours] of Object.entries(weeks)) {
      if (hours > ctx.horasSemanalesMax) {
        violations.push({
          tipo: "horas_semanales_excedidas",
          worker_rut: rut,
          detalle: `Semana ${wk}: ${hours}h > ${ctx.horasSemanalesMax}h`,
        });
      }
    }
  }
  ...
  for (const [rut, worked] of Object.entries(workedByRut)) {
    if (worked > maxWorked) {
      violations.push({
        ...
        worker_rut: rut,
        detalle: `Trabaja ${worked} domingos; maximo permitido ${maxWorked}`,
      });
    }
  }
    ...
    for (let t = apertura; t < cierre; t += 30) {
      ...
      if (count < ctx.coberturaminima) {
        violations.push({
          ...
          detalle: `${date} ${Math.floor(t / 60)}:${String(t % 60).padStart(2, "0")}: ${count} trabajadores (minimo ${ctx.coberturaminima})`,
        });
        break;
      }
    }
    ...
    for (const vac of vacsByRut[a.worker_rut] ?? []) {
      if (a.date >= vac.desde && a.date <= vac.hasta) {
        violations.push({
          ...
          worker_rut: a.worker_rut,
          detalle: `${a.worker_rut} esta de vacaciones el ${a.date}`,
        });
      }
    }
