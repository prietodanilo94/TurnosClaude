...
def _workers_in_slot(slot_start: int, date: str,
                     by_date: Dict[str, List[ShiftInfo]]) -> int:
    return sum(
        1 for s in by_date.get(date, [])
        if s.inicio_min <= slot_start < s.fin_min
    )
...
def compute_metrics(solution: SolverOutput, inp: SolverInput) -> ProposalMetrics:
    ...
    for asig in solution.asignaciones:
        ...
        if shift is None:
            continue
        horas_por_worker[asig.worker_rut] = horas_por_worker.get(asig.worker_rut, 0.0) + shift.duracion_h
        by_date.setdefault(asig.date, []).append(shift)
        if shift.duracion_h <= 7.0:
            turnos_cortos += 1
    ...
    for day in inp.days:
        ...
        for slot in _slots_in_range(peak_start, day.cierre_min):
            peak_total += 1
            if _workers_in_slot(slot, day.date, by_date) >= 2:
                peak_cubiertos += 1
    ...
    for week_indices in inp.weeks:
        ...
        if not weekend:
            continue
        completo = all(
            _workers_in_slot(slot, day.date, by_date) >= cobertura_minima
            for day in weekend
